...
async def resolve_get_book_by_isbn(isbn: str) -> GetResponse[Book]:
    return await book_service.get_book_by_isbn(ObjectId(isbn))


async def resolve_create_book(title: str, author: AuthorInput, price: float,
                              subtitle: str = None, description: str = None,
                              isbn: str = None, book_format: str = None) -> CreateResponse[Book]:
...
async def resolve_update_book(book_id: str,
                              title: str = None,
                              author: AuthorInput = None, price: float = None,
                              subtitle: str = None, description: str = None,
                              isbn: str = None, book_format: str = None,
                              rating: float = None) -> UpdateResponse[Book]:
    new_values = {}
    ...
    if author is not None:
        new_values['author'] = author.to_dict()
    ...
    if book_format is not None:
        new_values['book_format'] = book_format
    ...
    new_values['lastUpdateOn'] = datetime.datetime.now()

    return await book_service.update_book(book_id, new_values)
